...
from .models import Universite, Faculte, Departement, Niveaux
from .forms import UniversiteForm, FaculteForm, DepartimentForm, NiveauxForm
...
def all_classes(request):
      niveau_data=Niveaux.objects.all()
      if 'Search' in request.GET:
            ...
            if Search=="":
                  ...
                  niveau_data=Niveaux.objects.filter()

      countd = Niveaux.objects.count()
      return render(request,"classe/all_classes.html",{'niveau_data': niveau_data,'countdt': countd})

def create_class(request):
      form=NiveauxForm
      if request.method == 'POST':
            form=NiveauxForm(request.POST)
            if form.is_valid():
                  form.save()
...
def update_class(request,pk):
      facult_data=Niveaux.objects.get(id=pk)
      form=NiveauxForm(instance=facult_data)
      if request.method == 'POST':
            form=NiveauxForm(request.POST,instance=facult_data)
            if form.is_valid():
                  form.save()
            return redirect('/niveaux')  # 4
      else:
...
def delete_class(request,Id):
      aut_data=Niveaux.objects.get(id=Id)
      aut_data.delete()
